...
We have $\alpha_n = \alpha_0 + \overbrace{1 + 1 + \dotsb + 1}^{n \;terms} = \overbrace{1 + 1 + \dotsb + 1}^{n \;terms}$.
Therefore, $\alpha_m\alpha_n = \alpha_m(\overbrace{1 + 1 + \dotsb + 1}^{n \;terms}) = \overbrace{\alpha_m +\alpha_m + \dotsb + \alpha_m}^{n \;terms} =\\ \overbrace{(1 + 1 + \dotsb + 1) + (1 + 1 + \dotsb +1) + \dotsb + (1 + 1 + \dotsb + 1)}^{n \;terms} = \overbrace{1 + 1 + \dotsb + 1}^{mn \;terms} = \alpha_{mn}$\\\\
Assume there exists an $n$ with $\alpha_n = 0$ but $\alpha_k \ne 0$ for any $k < n$.
It suffices to prove that $n$ is a prime.\\
% ここに証明②
Suppose n is not any prime. Let p be one of prime factors of $n$ and then we have $n = p \cdot_\Z p' (p' \in\mathbb{N}$ and $p' > 1)$.
